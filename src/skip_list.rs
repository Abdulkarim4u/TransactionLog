//Improving our transactionLog by using more robust Algorithms.
//Adding the following features
//1.A time associated with the logged transaction
//2.To be able to jump to an arbitrary time.
//3.To start iterating from there.
//This is a perfect fit for a skiplist.
...
//using skiplist we have different levels, and a Node having different successor at every level, the next pointer should be an array.
//The tail property is a vector pointing to the tail if each level.
...
impl BestTransactionLog {
    ...
    pub fn find(&self, offset: u64) -> Option<String> {
        match self.head {
            Some(ref head) => {
                let mut start_level = self.max_level; //starting at the highest possible level.
                let node = head.clone();
                let mut result = None;
                loop {
                ...
                //move vertically toward the desired node and then move lower.
                let mut n = node;
